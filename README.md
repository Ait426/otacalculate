...
middleware.js   ★ 접근 보호. 비밀번호 없이는 HTML 이 나가지 않는다
vercel.json
```
...
이 사이트에는 프로모션 ID와 적용 상태, 보수 구성, 채널별 CMS, 입금가 하한,
실제 예약 건이 들어 있습니다. **URL만 알면 누구나 볼 수 있게 두면 안 됩니다.**

`middleware.js` 가 Edge 에서 Basic 인증을 겁니다. 비밀번호를 모르면 HTML 이
아예 전송되지 않습니다. 브라우저 자바스크립트로 막는 방식은 파일이 이미
내려간 뒤라 소스 보기 한 번에 뚫리므로 쓰지 않았습니다.
...
둘 다 **Production** 에 넣고 저장한 뒤 **Deployments → 최신 배포 → Redeploy**.
환경변수는 배포 시점에 주입되므로 저장만 해서는 반영되지 않습니다.

> 두 값이 하나라도 비어 있으면 사이트를 열어두지 않고 **503 으로 잠급니다.**
> 설정을 빠뜨린 채 요금 전략이 공개되는 것보다 잠깐 안 열리는 편이 낫기 때문입니다.

비밀번호를 바꾸려면 환경변수를 고치고 다시 Redeploy 하면 됩니다.
`dist/index.html` 을 파일로 직접 열 때는 인증이 걸리지 않습니다 —
그 파일을 남에게 보내면 내용이 그대로 나가므로 주의하세요.
