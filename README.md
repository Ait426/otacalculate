...
> 🔓 **인증 없이 공개돼 있습니다.** 프로모션 설정·보수 구성·채널별 CMS·입금가 하한이
> URL을 아는 사람에게 그대로 보입니다. `noindex` 라 검색에는 안 잡히지만
> **주소를 아무에게나 주지 마세요.** 2026-08-24 에 Edge Middleware 로 Basic 인증을
> 걸었다가, 여러 사람이 쓰는 편의를 위해 걷어냈습니다 (커밋 `41a4dc9` → `revert`).
> 다시 막으려면 그 커밋을 되살리고 `SITE_USER` / `SITE_PASS` 를 넣으면 됩니다.
